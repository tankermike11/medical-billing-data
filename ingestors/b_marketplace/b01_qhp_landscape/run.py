"""QHP Landscape File 2025 ingestor.

Source: CMS/CCIIO via data.healthcare.gov — Individual Market Medical (Excel).
Row 0 is a metadata summary row; Row 1 contains column headers; Row 2+ is data.

Key columns used:
  State Code
  FIPS County Code
  County Name
  Plan ID (Standard Component)   — 14-char HIOS ID
  Plan Marketing Name
  Metal Level
  Plan Type
  Issuer Name
  HIOS Issuer ID
  Summary of Benefits URL        — SBC link (critical for Family F)
  Plan Brochure URL
  Network URL
"""
import zipfile
...
SOURCE_ID = "b01_qhp_landscape"
PLAN_YEAR = 2025


def _build_col_map(header_row: tuple) -> dict[str, int]:
    """Normalize headers to lowercase_with_underscores → column index."""
    return {
...
def ingest(source: dict, local_path: Path, file_hash: str, conn: sqlite3.Connection) -> int:
    ...
    with zipfile.ZipFile(local_path) as zf:
        ...
        if not xlsx_name:
            raise FileNotFoundError("No .xlsx file found in QHP Landscape ZIP")
    ...
    next(rows_iter)  # skip row 0 — metadata summary row
    header_row = next(rows_iter)  # row 1 — actual column headers
    ...
    for row in rows_iter:
        ...
        state      = _get(row, cm, "State Code", "State", "StateCode")
        ...
        issuer_id   = _get(row, cm, "HIOS Issuer ID", "Issuer_ID", "IssuerID", "IssuerId")
